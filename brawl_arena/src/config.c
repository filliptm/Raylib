...
static void SetStatus(World *w, const char *message)
{
...
static bool SaveProjectSnapshot(World *w)
{
    char validation[160];
...
static bool SaveLocalDraft(World *w)
{
    char temporary[512];
    ...
    int liveCount = BuildFields(&w->tune, WEAPONS, live);
...
static bool SaveProfile(World *w)
{
    char temporary[512];
    ...
    Field fields[MAX_FIELDS];
    int count = BuildFields(&w->tune, WEAPONS, fields);
    for (int i = 0; i < count; i++)
...
bool ConfigInitialize(World *w)
{
    ...
    memcpy(candidateWeapons, WEAPONS, sizeof(candidateWeapons));
    ...
    if (!projectOk)
    {
        ...
        memcpy(w->config.projectWeapons, WEAPONS, sizeof(w->config.projectWeapons));
    ...
    memcpy(WEAPONS, candidateWeapons, sizeof(candidateWeapons));
    ...
    if (PathExists(LocalPath()))
    {
        ...
        memcpy(localWeapons, WEAPONS, sizeof(localWeapons));
        if (LoadTypedFile(LocalPath(), &localTuning, localWeapons,
                          SCOPE_LOCAL, false, message, sizeof(message)))
        {
            w->tune = localTuning;
            memcpy(WEAPONS, localWeapons, sizeof(localWeapons));
            localLoaded = true;
        }
    ...
    if (PathExists(ProfilePath()))
    {
        ...
        memcpy(unchangedWeapons, WEAPONS, sizeof(unchangedWeapons));
...
bool ConfigPromoteKit(World *w, BrawlerClass cls)
{
    ...
    w->config.projectWeapons[cls] = WEAPONS[cls];
...
void ConfigResetKitToProject(World *w, BrawlerClass cls)
{
    ...
    WEAPONS[cls] = w->config.projectWeapons[cls];
    ConfigMarkDirty();
