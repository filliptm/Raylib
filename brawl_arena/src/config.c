...
static int BuildFields(World *w, Field *f)
{
    ...
    n = AddField(f, n, "stat_wins",      F_INT,   &t->statWins);
    n = AddField(f, n, "stat_losses",    F_INT,   &t->statLosses);
    n = AddField(f, n, "stat_kos",       F_INT,   &t->statKos);
    n = AddField(f, n, "gem_grab",       F_BOOL,  &t->gemGrab);
    n = AddField(f, n, "team_size",      F_INT,   &t->teamSize);
    n = AddField(f, n, "gems_to_win",    F_INT,   &t->gemsToWin);
    n = AddField(f, n, "gem_countdown",  F_FLOAT, &t->gemCountdown);
    n = AddField(f, n, "gem_interval",   F_FLOAT, &t->gemVentInterval);
...
bool ConfigLoad(World *w)
{
    ...
    if (t->selectedKit < 0 || t->selectedKit >= CLASS_COUNT) t->selectedKit = 0;
    if (t->statWins < 0) t->statWins = 0;
    if (t->statLosses < 0) t->statLosses = 0;
    if (t->statKos < 0) t->statKos = 0;
    if (t->teamSize < 1) t->teamSize = 1;
    if (t->teamSize > MAX_BRAWLERS/2) t->teamSize = MAX_BRAWLERS/2;
    if (t->gemsToWin < 1) t->gemsToWin = 1;
    if (t->gemCountdown < 1.0f) t->gemCountdown = 1.0f;
    if (t->gemVentInterval < 0.5f) t->gemVentInterval = 0.5f;
